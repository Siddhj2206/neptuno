...
| Build fails: docker-ce repo 404 (`download.docker.com/linux/fedora/<N>/...`) after a Fedora major bump | Docker adds a `fedora/<N>` dir only after its `docker/packaging` PR merges **and** a component release is cut — historically 2–12 weeks before GA, on no fixed schedule; `build/45-dx.sh` derives its `baseurl` from `$releasever`, which a fresh Branched base sets to a major Docker may not serve yet | Probe `https://download.docker.com/linux/fedora/<N>/` and the open `docker/packaging` PR before bumping; until the dir appears, pin the docker-ce `baseurl` to the newest Fedora Docker serves (or drop the docker section). COPRs and negativo17 publish Branched chroots earlier — probe them separately |
...
| CI build fails at "Rechunk image": `tee: /var/tmp/chunkah-config.XXXXXX.json: Permission denied` | `bootc-build/chunka` (projectbluefin/actions) creates its config temp file as the runner user in sticky `/var/tmp`, then writes it with `sudo tee`. The kernel's `fs.protected_regular` check denies root an `O_CREAT` open of a file it does not own in a sticky world-writable dir | Upstream fix is `sudo mktemp` (root-owned file). Local workaround: `sudo sysctl -w fs.protected_regular=0` in a step before rechunk. Remove once the action pin moves past the fix — see the note in `build-image.yml` |
...
| Boot reaches emergency mode with `/var` mount timeouts and `rechunker-group-fix.service` ordering-cycle logs | The inherited legacy-rechunker unit's `Before=systemd-sysusers.service` conflicts with its `After=local-fs.target`; systemd can drop `/var` mount jobs to break the cycle. An empty `Before=` in a drop-in does not remove the main unit's ordering edge | Images that have never used legacy rechunker should disable the unit and vendor-mask it at `/usr/lib/systemd/system/rechunker-group-fix.service`; verify `systemctl is-enabled rechunker-group-fix.service` reports `masked` and the boot journal has no ordering cycle |
...
| "A shared projectbluefin/actions regression will show up in bluefin's CI." | It often won't. bluefin/bluefin-lts gate rechunk behind `stream_name != testing` and always pass `testing`; dakota uses its own `just chunkify`; ublue-os/bluefin uses `just rechunk` and ublue-os/bluefin-lts uses legacy-rechunk. No projectbluefin repo executes `bootc-build/chunka`, so a chunka regression only surfaces in finpilot forks with `ENABLE_RECHUNKING: "true"`. Report or PR it upstream yourself — do not assume someone else's pipeline will catch it. |
