...
| Boot reaches emergency mode with `/var` mount timeouts and `rechunker-group-fix.service` ordering-cycle logs | The inherited legacy-rechunker unit's `Before=systemd-sysusers.service` conflicts with its `After=local-fs.target`; systemd can drop `/var` mount jobs to break the cycle | Keep the migration unit, but ship `custom/files/usr/lib/systemd/system/rechunker-group-fix.service.d/10-neptuno-ordering.conf` with `[Unit]` then `Before=`; verify `journalctl -b -u rechunker-group-fix.service` has no ordering-cycle or tmpfiles write errors |
