...
| Build fails: third-party repo 404 after a Fedora major bump | Vendor repos (Tailscale in `base.toml`, negativo17 in `multimedia.toml`) may not publish a `$releasever`/`fedora/<N>` chroot the moment Branched opens; their repo URLs derive from `$releasever` | Probe the vendor path for the new major before bumping; if absent, pin the repo `baseurl` to the previous major or defer the bump. COPRs publish Branched chroots early — probe them separately |
...
| Podman not found                       | Container runtime not installed                                  | Install `podman`; it is the image's container runtime                          |
