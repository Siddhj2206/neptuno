...
| Build fails: docker-ce repo 404 (`download.docker.com/linux/fedora/<N>/...`) after a Fedora major bump | Docker's official Fedora repo publishes GA majors only, so a pre-GA (Branched) base has no docker dir; `build/45-dx.sh` derives its `baseurl` from `$releasever` (base os-release) | Pin the docker-ce `baseurl` to the newest Fedora Docker actually serves (or drop the docker section) until Docker publishes the new major. COPRs and negativo17 DO publish Branched chroots — probe `.../fedora-<N>/` before blaming them |
