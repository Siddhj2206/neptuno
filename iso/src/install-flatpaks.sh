...
# work but is small — too small for the GNOME runtime. The --mount=type=cache
# volume is a bind-mount from btrfs (supports O_TMPFILE) and has plenty of
...
if [[ "${INSTALLER_CHANNEL:-stable}" == "dev" ]]; then
    ...
    # the same /releases/latest/ redirect the stable channel uses.
...
# List is at /tmp/flatpaks-list (COPYd by the Containerfile) — the neptuno
# essentials (see iso/src/flatpaks). The installer itself is kept regardless.
