...
| `image-tag` | `"stable"` (or `gts`, `latest`, `stable-daily`) (`bluefin/Containerfile:61`) | `"stable"` (`pluto/Containerfile:61`) | ✅ | ✅ Correct. `UBLUE_IMAGE_TAG` receives `DEFAULT_TAG` from the main-only build workflow, and `Justfile:132` correctly wires `UBLUE_IMAGE_TAG=${tag}` (`:132`). | **Correct** |
...
* **`image-tag` / `UBLUE_IMAGE_TAG` plumbing** — `Containerfile:61` `ARG UBLUE_IMAGE_TAG="stable"`, `Justfile:132` `UBLUE_IMAGE_TAG=${tag}`, and the main-only `build-image.yml` workflow build the local `pluto:stable` image before publishing its `:stable-daily` aliases.
