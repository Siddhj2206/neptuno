...
* **`image-tag` / `UBLUE_IMAGE_TAG` plumbing** — `Containerfile:61` `ARG UBLUE_IMAGE_TAG="stable"`, `Justfile:132` `UBLUE_IMAGE_TAG=${tag}`, and the main-only `build-image.yml` workflow all produce the `pluto:stable` image tag.
